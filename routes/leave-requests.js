...
router.get('/', auth.verifyToken, async (req, res) => {
  try {
    ...
    let query = `
      SELECT lr.*, s.name as student_name, s.student_id
...
router.post('/', auth.verifyToken, async (req, res) => {
  try {
    const { start_date, end_date, reason, leave_type = 'Personal' } = req.body;
    const studentId = req.user.id;
    
    if (!studentId || studentId === 'undefined') {
      return res.status(400).json({ message: 'You must be logged in as a student to create a leave request' });
    }
    ...
    const insertQuery = `
      INSERT INTO leave_requests (student_id, start_date, end_date, reason, leave_type, status, created_at, updated_at)
      VALUES ($1, $2, $3, $4, $5, 'pending', NOW(), NOW())
      RETURNING *
    `;
    
    const result = await db.query(insertQuery, [studentId, start_date, end_date, reason, leave_type]);
